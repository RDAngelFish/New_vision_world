...
int vlsi_esp_waiting(VLSI_ESP *obj)
{
    ...
    while(f == -2)
    {
        for(int i = 0; i < 20; i++) 
        {
            ...
            if((strstr(esp_rx_buf, "OK") != NULL) || (strstr(esp_rx_buf, "ERROR") != NULL))
            {
//                EMBARC_PRINTF("break\r\n");
//                
//                EMBARC_PRINTF("--------------------------\r\n");
...
int vlsi_esp_send_uint(VLSI_ESP *obj, unsigned int data_u32)
{
    ...
    data_u8[0] = (unsigned char)(data_u32 % 256);               // 0 ~  7
    data_u8[1] = (unsigned char)((data_u32 / 256) % 256);       // 8 ~ 15
    data_u8[2] = (unsigned char)((data_u32 / 65536) % 256);     //16 ~ 23
    data_u8[3] = (unsigned char)((data_u32 / 16777216) % 256);  //24 ~ 31
