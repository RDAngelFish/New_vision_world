...
int vlsi_ftdi_receive(VLSI_FTDI *obj, int *buf)//0-3 vlsi 4-171 84*2 172 O 173 K
{
    while((ftdi_rx_buf[172] != 'O') || (ftdi_rx_buf[173] != 'K'))       //Need time out
    {
        board_delay_ms(5,1);
    }
    ...
    for(int i = 0; i < 84; i++)
    {
        buf[i] = ((unsigned int)ftdi_rx_buf[2 * i + 4]) * 256 + (unsigned int)ftdi_rx_buf[2*i + 5];
    }   
