...
print(iris.DESCR)
X_d = iris.data
X = np.loadtxt('output/data/data_200.txt')
Y = np.loadtxt('output/data/data_target_200.txt')
...
while True:
   ...
   #          landmarks = np.matrix([[p.x, p.y] for p in predictor(cframe_data, rects[i]).parts()])
   ...
   # # cv2.imshow("img", cframe_data)
    ...
    horizontal_line_color_175 = (255, 255, 255)
    cv2.line(output_img, (0, 175), (640, 175), horizontal_line_color_175, 1)
...
#
#         if counter == 200:
#             np.savetxt('output/data/b_data.txt', data, fmt='%.2f')
#             np.savetxt('output/data/b_data_target.txt', data_target, fmt='%d')
#             print('save data')
#             depth_stream.stop()
#             color_stream.stop()
#             dev.close()
...
##################################### Dump data ###################################
    ...
    if int(key) == 122:
        if datum.handKeypoints[1][0][0][0] == 0 and datum.handKeypoints[1][0][0][1] == 0 and datum.handKeypoints[1][0][0][2] == 0:
            print('cannot find right hand')
        else:
